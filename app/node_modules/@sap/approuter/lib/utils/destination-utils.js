...
function getDestinationToken(options, token, credentials, cb) {
  ...
  // Prefer user token in case not local destination requested
  if (options.userToken && !options.preferLocal) {
    return cb(null, options);
  }
  // Fetch tenant from url or session
  let tenant = options.urlTenant || (options && options.session && options.session.user && options.session.user.tenant);

  // If no tenant, take destination client credentials token (public access or not multi-tenant approuter or SaaS Approuter APIs)
  if (!tenant && options && options.app) {
  ...
  else if (tenant && options && options.app) {
    if (options.app.services[DESTINATION]
      && options.app.services[DESTINATION] && options.app.services[DESTINATION][tenant]
