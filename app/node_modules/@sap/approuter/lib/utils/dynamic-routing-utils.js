...
function getRemoteConfiguration(req, res, next) {
  ...
  if (isHTML5RepoExcludedApp(appKey, req)) {
...
function storeLogoutDataOnRequest(req, xsapp) {
  let routerConfigServices = req.app && req.app._store && req.app._store.mainRouterConfig && req.app._store.mainRouterConfig.appConfig
  ...
  let approuterHost = req.headers.host ? req.headers.host : null;
...
function getCacheBusterToken(req) {
  let suspectSegment = req.url.split('/')[2];
  let tokenRegEx = /^~(.*)~$/;
...
function getApplicationKey(req) {
  let appKey = {};
  let parts;
  let appNameRegex = new RegExp('^[a-zA-Z0-9_ ]+$');
  ...
  return appKey && appNameRegex.test(appKey.appNameWithoutPrefix) ? appKey : null;
}
...
function fetchSubscriberSubaccountToken(req, tenant, html5RepoCredentials, cb) {
  if (req.app.services[html5RepoService] && req.app.services[html5RepoService][tenant]
